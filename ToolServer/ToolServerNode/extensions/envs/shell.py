...
def read_pipe(pipe: Union[io.StringIO, io.BytesIO], text=True) -> Union[str, bytes]:
    ...
    while True:
        ...
        if len(ready_fds) == 0:
            break
        output += (
            os.read(ready_fds[0], 16384).decode()
            if text
            else os.read(ready_fds[0], 16384)
        )
...
class ShellEnv(BaseEnv):
    ...
    def _restart(self, program: str = None, shell: bool = True):
        ...
        self.output_fileno = [
            self.running_proc.stdout.fileno(),
            self.running_proc.stderr.fileno(),
        ]
    ...
    def read_stdout(self, probe: bool = False) -> str:
        ...
        if probe and len(ready_fds) == 0:
            raise OutputNotReady(
                "Output is not ready!",
                next_calling=get_func_name(self.read_stdout, self),
