...
class CommandLine:
    def __init__(self, env: XAgentServerEnv):
        ...
        self.logger.typewriter_log(
            title=f"XAgentServer is running on cmd mode", title_color=Fore.RED
        )
        self.logger.info(
            title=f"XAgentServer log:", title_color=Fore.RED, message=f"{self.log_dir}"
        )
        ...
        if env.DB.db_type in ["sqlite", "mysql", "postgresql"]:
            ...
            from XAgentServer.database.lsi import InteractionLocalStorageInterface

            self.logger.info("init localstorage connection: interaction.json")
    ...
    def run(self, args: dict):
        ...
        io = XAgentIO(
            input=CommandLineInput(
                do_interrupt=base.mode != "auto", max_wait_seconds=max_wait_seconds
            ),
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2:
        ...
        if len(sys.argv) >= 3:
            ...
            sys.stdout = open(
                os.path.join(recorder.record_root_dir, "command_line.ansi"),
                "w",
                encoding="utf-8",
            )
