...
@toolwrapper()
async def shell_command_executor(
    command: str = "", run_async: bool = False, shell_id: int = None, kill: bool = False
):
    ...
    if shell_id is not None:
        ...
        if exec_proc is None:
            raise ToolExecutionError({"Error": "Shell not found or has been closed."})
    ...
    if not run_async:
        try:
            ret = await asyncio.wait_for(
                exec_proc.communicate(command.encode()),
        ...
        result = {"ReturnCode": exec_proc.returncode, "display": ""}
        if ret[1] != b"":
            result["display"] += f"\nstderr:\n" + ret[1].decode()
        if ret[0] != b"":
            result["display"] = f"\nstdout:\n" + ret[0].decode()
