...
def parse_args():
    ...
    parser.add_argument("--upload_files", nargs="+", help="upload files")
    parser.add_argument(
        "--model",
        type=str,
    )
    ...
    parser.add_argument(
        "--config_file", type=str, default=os.getenv("CONFIG_FILE", "assets/config.yml")
    )
...
if __name__ == "__main__":
    ...
    for key, value in args.items():
        if value is not None:
            if key == "model":
                ARGS["default_completion_kwargs"] = deepcopy(
