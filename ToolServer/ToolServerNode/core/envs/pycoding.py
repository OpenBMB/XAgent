...
class PythonNotebook(BaseEnv):
    ...
    async def execute_cell(
        self, code: str, cell_index: int = None, reset: bool = False
    ) -> str:
        ...
        if (
            cell_index is None
            or cell_index == len(self.nb.cells)
            or len(self.nb.cells) == 0
        ):
        ...
        nbformat.write(
            self.nb, os.path.join(self.work_directory, self.nb_cfg["save_name"])
        )

        return self._format_outputs(
            self.nb.cells[cell_index].outputs,
            cell_index,
            reraise=True,
            return_binary=True,
        )
    ...
    def _format_outputs(
        self, outputs, cell_index=None, reraise=False, return_binary=False
    ):
        ret = None
        if len(outputs) == 0:
            ...
            if cell_index is not None:
                ret = {
                    ...
                    "data": [
                        ...
                        self._format_output(
                            outputs[0], cell_index, reraise, return_binary
                        ),
                    ],
                }
            else:
                ret = self._format_output(
                    outputs[0], cell_index, reraise, return_binary
                )
        else:
            ret = {
                "type": "composite",
                "data": [
                    self._format_output(output, cell_index, reraise, return_binary)
    ...
    def _format_output(
        self, output, cell_index=None, reraise=False, return_binary=False
    ):
