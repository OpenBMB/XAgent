...
from XAgent.agent.summarize import (
    summarize_action,
    summarize_plan,
    clip_text,
    get_token_nums,
...
def make_message(now_node: ToolNode, task_handler, max_length, config):
    if CONFIG.enable_summary:
        terminal_task_info = summarize_plan(task_handler.now_dealing_task.to_json())
    else:
...
class ReACTChainSearch(BaseSearchMethod):
    ...
    async def run_async(
        ...
        for _attempt_id in range(max_try):
            await self.generate_chain_async(
                config, agent, task_handler, arguments, functions, task_id
            )
    ...
    def rewrite_input_func(self, old, new):
        ...
        if new is None:
            ...
            if assistant_thoughts:
                ...
                if (
                    "reasoning" in args.keys()
                    and "reasoning" in assistant_thoughts.keys()
                ):
                ...
                if (
                    "criticism" in args.keys()
                    and "criticism" in assistant_thoughts.keys()
                ):
    ...
    async def generate_chain_async(
        self, config, agent: BaseAgent, task_handler, arguments, functions, task_id
    ):
        ...
        while now_node.get_depth() < config.max_subtask_chain_length:
            ...
            if now_node.father != None:
                if task_handler.interaction.interrupt:
                    ...
                    receive_data = await task_handler.interaction.auto_receive(
                        can_modify
                    )
            ...
            (
                file_archi,
                _,
            ) = toolserver_interface.execute_command_client(
            ...
            new_message, tokens = agent.parse(
                placeholders={
                    ...
                    "user": {
                        ...
                        "subtask_id": task_handler.now_dealing_task.get_subtask_id(
                            to_str=True
                        ),
                        "max_length": config.max_subtask_chain_length,
            ...
            print_data = print_assistant_thoughts(new_tree_node.data, False)
            ...
            (
                ...
                using_tools,
            ) = function_handler.handle_tool_call(new_tree_node, task_handler)
            self.need_for_plan_refine = need_for_plan_refine

            now_attempt_tree.make_father_relation(now_node, new_tree_node)

            await task_handler.interaction.update_cache(
                update_data={**print_data, "using_tools": using_tools},
                status="inner",
                current=task_id,
            )
