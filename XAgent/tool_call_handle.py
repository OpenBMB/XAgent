        ...
        if is_wrapped_response(obj):
            match obj["type"]:
                ...
                case "binary":
                    ...
                    if obj["media_type"] == "image/png" and not str(name).endswith(
                        ".png"
                    ):
                        name += ".png"
                    with open(os.path.join("local_workspace", name), "wb") as f:
                        f.write(base64.b64decode(obj["data"]))
                    return {"media_type": obj["media_type"], "file_name": name}
                case "composite":
...
class ToolServerInterface:
    ...
    def get_json_schema_for_tools(
        self,
        command_names,
    ):
        url = f"{self.url}/get_json_schema_for_tools"
        payload = {"tool_names": command_names}
        cache_output = recorder.query_tool_server_cache(url, payload)
        try:
            if cache_output != None:
                response = cache_output["tool_output"]
...
class FunctionHandler:
    ...
    def log_task_submit(self, arguments):
        ...
        if "milestones" in arguments["result"].keys():
            logger.typewriter_log(f"milestones:", Fore.YELLOW)
    ...
    def get_functions(self, config):
        ...
        openai_function_jsons = [
            ...
            for openai_function_json in openai_function_jsons
            if openai_function_json["name"] not in black_list
        ]
        ...
        for tool_json in openai_function_jsons:
            function_manager.register_function(tool_json)
        return (
            self.intrinsic_tools(config.enable_ask_human_for_help),
            self.avaliable_tools_description_list,
        )
    ...
    def handle_tool_call(self, node: ToolNode, task_handler):
        ...
        if command_name == "subtask_submit":
            ...
            while (
                ...
                and tool_output_status_code == ToolCallStatusCode.TIMEOUT_ERROR
                and isinstance(command_result["detail"], dict)
                and "type" in command_result["detail"]
                and command_result["detail"]["type"] == "retry"
            ):
                time.sleep(3)
                retry_time += 1
                (
