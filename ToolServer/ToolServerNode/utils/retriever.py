...
def build_tool_embeddings(tools_json: list[dict]):
    ...
    if os.path.exists(cfg["id2tool_file"]) and os.path.exists(cfg["embedding_file"]):
        ...
        if len(id2tool) != len(doc_embedings):
            logger.error(
                "Embedding file and id2tool file do not match! Rebuild embeddings!"
            )
            id2tool = {}
            doc_embedings = []
    else:
    ...
    def get_embedding(tool_json: dict):
        ...
        try:
            ...
            logger.error(
                f'Failed to get embedding for tool {tool_json["name"]}! Error: {e}'
            )
            return [-1.000001] * cfg["embedding_dim"]
        return response.json()["data"][0]["embedding"]

    uncached_tools = list(
        filter(lambda tool_json: tool_json["name"] not in cached_tool_names, tools_json)
    )
    uncached_tools_name = list(map(lambda tool_json: tool_json["name"], uncached_tools))
    uncached_doc_embedings = []
    with ThreadPoolExecutor(16) as pool:
        futures = [
            pool.submit(get_embedding, tool_json) for tool_json in uncached_tools
        ]
    ...
    for tool_json in tools_json:
        if tool_json["name"] not in cached_tool_names:
            ...
            new_doc_embedings.append(
                uncached_doc_embedings[uncached_tools_name.index(tool_json["name"])]
            )
        else:
