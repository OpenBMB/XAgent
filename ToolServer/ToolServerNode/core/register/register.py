...
class ToolRegister:
    def __init__(
        ...
        for k in self.toolregister_cfg["tool_creation_context"]:
            # load
            load_code = (
                f"from {self.toolregister_cfg['tool_creation_context'][k]} import {k}"
            )
            exec(load_code)
        ...
        # load extensions
        if "enabled_extensions" in self.config.cfg and isinstance(
            self.config["enabled_extensions"], list
        ):
    ...
    def register_tool(self, tool_name: str, code: str) -> str:
        ...
        try:
            ...
            raise ToolRegisterError(
                f"Failed to find tool, please verify the tool_name!",
                tool_name=tool_name,
            )
        ...
        if tool_func is None:
            raise ToolRegisterError(
                f"Tool: {tool_name} has no labels or replicated! Ensuring wrap the tool with `@toolwrapper()`.",
                tool_name=tool_name,
            )
    ...
    def get_env_dict(self, env_name: str) -> dict:
        ...
        return self.envs[env_name].env_labels.dict(
            include_invisible=True, max_show_tools=-1
        )
    ...
    def get_all_tools(self, include_invisible=False) -> list[str]:
        if include_invisible:
            ...
            return [
                ...
                if self.tools[tool_name].tool_labels.visible
            ]

    def get_all_tools_dict(self, include_invisible=False) -> list[dict]:
        return [
            self.tools[tool_name].tool_labels.dict(name_overwrite=tool_name)
            for tool_name in self.get_all_tools(include_invisible)
        ]
        ...
        if isinstance(key, str):
            if key not in self.tools:
                ...
                try:
                    ...
                    if (
                        self.dynamic_extension_load(f"extensions.tools.{key}")
                        and key in self.tools
                    ):
            ...
            if env_name not in self.envs:
                ...
                if (
                    self.dynamic_extension_load(f"extensions.envs.{env_name}")
                    and env_name in self.envs
                ):
            ...
            if tool_name not in env.env_labels.subtools_labels:
                ...
                func = getattr(
                    env, env.env_labels.subtools_labels[tool_name].method.__name__
                )
                return func
        raise NotImplementedError(f"Key {key} is not valid!")
