...
def toolwrapper(
    ...
    def decorator(obj: object) -> Union[Type, Callable[..., Any]]:
        if inspect.isclass(obj):
            ...
            if not visible:
                description = (
                    "Note: All tools of this env are invisible during all tools display, please check this env's defination to show all tools.\n"
                    + description
                )
            ...
            if BaseEnv not in cls.__bases__:
                ...
                if not parent_tools_visible:
                    description = (
                        f"Note: This env is subclass of {direct_parents}, and all tools of parent envs are inherited and not visible. You can try call parent tools or check this env's defination to show them.\n"
                        + description
                    )
                else:
                    description = (
                        f"Note: This env is subclass of {direct_parents}, and all tools of parent envs are inherited.\n"
                        + description
                    )
                for parent in cls.__bases__:
                    if hasattr(parent, "env_labels") and isinstance(
                        parent.env_labels, EnvLabels
                    ):
            ...
            for func_name in cls_func_names:
                ...
                if inspect.ismethod(origin_func) and not inspect.isfunction(
                    origin_func
                ):
