    ...
    async def update_cache(self, update_data: dict, status="", current: str = None):
        ...
        if status == "start":
            for k, v in update_data.items():
                if k == "subtasks":
                    update_data["subtasks"] = [Subtask(**subtask) for subtask in v]
        ...
        if status == "subtask":
            if current is None:
                raise ValueError("current must be a task_id while status is subtask")
            if not isinstance(update_data, list):
                raise ValueError("update_data must be a list while status is subtask")
            new_subtask_list = []
            ...
            new_subtask_list.extend([Subtask(**subtask) for subtask in update_data])
