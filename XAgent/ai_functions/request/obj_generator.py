...
class OBJGenerator:
    ...
    def schema_validation(self, schema: dict, to_validate: str):
        ...
        def validate():
            ...
            if "parameters" in schema:
                jsonschema.validate(
                    instance=to_validate["arguments"], schema=schema["parameters"]
                )
            # arguments validate
            else:
    ...
    def xagent_call_refine(self, req_kwargs, response):
        ...
        if (
            "function_call" not in response["choices"][0]["message"]["content"]
            and "arguments" not in response["choices"][0]["message"]["content"]
        ):
            logger.typewriter_log(
                "FunctionCallSchemaError: No function call or arguments in the response",
                Fore.RED,
            )
            # if with error msg
            if "broken_json" in response["choices"][0]["message"]["content"]:
                response = self.dynamic_xagent_jsons_fixs(
                    messages=req_kwargs["messages"],
                    functions=functions,
                    broken_jsons=[
                        response["choices"][0]["message"]["content"]["broken_json"]
                    ],
                    error_msgs=[
            ...
            raise FunctionCallSchemaError(
                f"No function call or arguments found in the response: {response['choices'][0]['message']['content']} "
            )
        ...
        if "function_call" in response["choices"][0]["message"]["content"]:
            ...
            function_schema = list(
                filter(
                    lambda x: x["name"]
                    == response["choices"][0]["message"]["content"]["function_call"][
                        "name"
                    ],
            ...
            if function_schema is None:
                ...
                if (
                    ...
                    not in req_kwargs["messages"][-1]["content"]
                ):
                ...
                logger.typewriter_log(
                    f"FunctionCallSchemaError: Function {response['choices'][0]['message']['content']['function_call']['name']} not found in the provided functions.",
                    Fore.RED,
                )
                raise FunctionCallSchemaError(
                    f"Function {response['choices'][0]['message']['content']['function_call']['name']} not found in the provided functions: {list(map(lambda x:x['name'],req_kwargs['functions']))}"
                )

            error_msg = self.schema_validation(
                schema=function_schema,
                to_validate=response["choices"][0]["message"]["content"][
                    "function_call"
                ],
            )
            if len(error_msg):
                ...
                if not isinstance(
                    response["choices"][0]["message"]["content"]["function_call"], str
                ):
                    broken_json = json5.dumps(
                        response["choices"][0]["message"]["content"]["function_call"]
                    )
                else:
                    broken_json = response["choices"][0]["message"]["content"][
                        "function_call"
                    ]
        ...
        if "arguments" in response["choices"][0]["message"]["content"]:
            ...
            error_msg = self.schema_validation(
                ...
                to_validate=response["choices"][0]["message"]["content"]["arguments"],
            )
            if len(error_msg):
                error_records.append(error_msg)
                if not isinstance(
                    response["choices"][0]["message"]["content"]["arguments"], str
                ):
                    broken_json = json5.dumps(
        ...
        if len(error_records):
            response = self.dynamic_xagent_jsons_fixs(
                messages=req_kwargs["messages"],
                functions=functions,
                broken_jsons=broken_jsons,
                error_msgs=error_records,
                function_schema=function_schema,
                arguments_schema=arguments_schema,
            )
        response["choices"][0]["message"]["content"] = json5.dumps(
            response["choices"][0]["message"]["content"]
        )
        return response
    ...
    def function_call_refine(self, req_kwargs, response):
        ...
        if "function_call" not in response["choices"][0]["message"]:
            ...
            raise FunctionCallSchemaError(
                f"No function call found in the response: {response['choices'][0]['message']} "
            )
