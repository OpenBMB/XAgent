...
def startup():
    ...
    app.doc_embeddings, app.id2tool = build_tool_embeddings(
        app.tool_register.get_all_tools_dict(include_invisible=True)
    )
...
async def download_file(
    file_path: str = Body(...), file_type: str = Body(default="text/plain")
):
    work_directory = CONFIG["filesystem"]["work_directory"]
...
async def get_workspace_structure():
    ...
    def generate_directory_structure(path):
        ...
        if os.path.isdir(path):
            ...
            result["children"] = [
                generate_directory_structure(os.path.join(path, child))
...
async def retrieving_tools(question: str = Body(...), top_k: int = Body(default=5)):
    try:
        retrieved_tools = ada_retriever(
            app.doc_embeddings, app.id2tool, question, top_k
        )
...
async def execute_tool(
    tool_name: str = Body(...),
    arguments: dict = Body(...),
    env_name: str = Body(default=None),
):
    tool_register: ToolRegister = app.tool_register
    ...
    try:
        ...
        logger.error(
            f"Error happens when executing tool {tool_name}! Exception: {e}\n{trace_info}"
        )
