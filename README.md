...
It is designed to be a general-purpose agent that can be applied to a wide range of tasks. Currently, XAgent is still in its early stages, and we are working hard to improve it.

🏆 Our goal is to create a super-intelligent agent that can solve any given task!

We welcome diverse forms of collaboration, including full-time, part-time roles, and more. If you are interested in the frontiers of agents and want to join us in realizing true autonomous agents, please contact us at xagentteam@gmail.com.
...
- **Cooperation with Humans**: XAgent can collaborate with you to tackle tasks. It not only has the capability to follow your guidance in solving complex tasks on the go but can also seek your assistance when it encounters challenges.
...
If you want to load from an existing record, set `record_dir` in config,default to `Null`. All the runs will be set to a record automatically. We remove `api key` and other unsafe items, so you can share your run with other people by sharing the records.
...
You could visit `http://localhost:5173` to interact with XAgent by using the web ui.
...
You can check out our live demo on [XAgent Official Website](https://www.x-agent.net/). We also provide a video demo and show cases of using XAgent here:
...
## 📊 Evaluation
We conduct human preference evaluations to evaluate XAgent's performance. We prepare over 50 real-world complex tasks for evaluation, which can be categorized into 5 classes: Search and Report, Coding and Developing, Data Analysis, Math, and Life Assistant.
