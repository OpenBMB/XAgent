    ...
    def parse(
        ...
        **kwargs,
        ...
        if self.config.default_request_type == "openai":
            ...
            functions = list(
                filter(
                    lambda x: x["name"] not in ["subtask_submit", "subtask_handle"],
                    functions,
                )
            )
            ...
            def change_tool_call_description(message: dict, reverse: bool = False):
                des_pairs = [
                    (
                        "Use tools to handle the subtask",
                ...
                for pair in des_pairs:
                    message["content"] = (
                        message["content"].replace(pair[0], pair[1])
                        if reverse
                        else message["content"].replace(pair[1], pair[0])
                    )
            ...
            functions = [
                function_manager.get_function_schema("subtask_submit"),
        ...
        if (
            self.config.default_request_type == "openai"
            and "tool_call" in function_call_args
        ):
            tool_schema = function_manager.get_function_schema(
                function_call_args["tool_call"]["tool_name"]
            )
            assert (
                tool_schema is not None
            ), f"Function {function_call_args['tool_call']['tool_name']} not found! Poential Schema Validation Error!"

            tool_call_args = (
                function_call_args["tool_call"]["tool_input"]
                if "tool_input" in function_call_args["tool_call"]
                else ""
            )
            ...
            def validate():
                ...
                if isinstance(tool_call_args, str):
                    tool_call_args = (
                        {} if tool_call_args == "" else json5.loads(tool_call_args)
                    )
                jsonschema.validate(
                    instance=tool_call_args, schema=tool_schema["parameters"]
                )
            ...
            message["function_call"]["arguments"] = message["function_call"].pop(
                "tool_input"
            )
