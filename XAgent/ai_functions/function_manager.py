...
class FunctionManager:
    def __init__(
        ...
        for cfg_file in glob.glob(
            os.path.join(self.function_cfg_dir, "*.yaml")
        ) + glob.glob(os.path.join(self.function_cfg_dir, "*.yml")):
            with open(cfg_file, "r") as f:
                function_cfg = yaml.load(f, Loader=yaml.FullLoader)
            self.function_cfgs[function_cfg["function"]["name"]] = function_cfg

        for cfg_file in glob.glob(
            os.path.join(self.pure_function_cfg_dir, "*.yaml")
        ) + glob.glob(os.path.join(self.pure_function_cfg_dir, "*.yml")):
            with open(cfg_file, "r") as f:
                function_cfg = yaml.load(f, Loader=yaml.FullLoader)
    ...
    def execute(
        self,
        function_name: str,
        return_generation_usage: bool = False,
        function_cfg: dict = None,
        **kwargs,
        ...
        completions_kwargs: dict = function_cfg.get(
            "completions_kwargs", CONFIG.default_completion_kwargs
        )
        ...
        match CONFIG.default_request_type:
            ...
            case "xagent":
                ...
                response = objgenerator.chatcompletion(
                    messages=messages, arguments=arguments, **completions_kwargs
                )
