...
STRING_INNER = r'(?:[^"\\\x00-\x1f\x7f-\x9f]|\\["\\/bfnrt]|\\u[0-9a-fA-F]{4})'
...
def _ref_resolver(schema: Dict) -> Callable[[str], Dict]:
    ...
    if "definitions" in schema:
        for definition, annotation in schema["definitions"].items():
            cache[f"#/definitions/{definition}"] = annotation
...
def expand_multi_type(expanded_property:Dict,  resolver: Callable[[str], Dict]):
    """
        specially recursively expand multi type
        replace the "ref" string
    """
    if "anyOf" not in expanded_property.keys():
        return
    for i,a in enumerate(expanded_property["anyOf"]):
        if "$ref" in a:
            expanded_property["anyOf"][i] = expand_json_schema(
                resolver(a["$ref"]),resolver
            )
        elif "type" in a and a["type"]=="array":
            expand_item_json_schema(a,resolver)
...
def expand_json_schema(
    ...
    if "properties" in raw_schema:
        ...
        for name, value in raw_schema["properties"].items():
            ...
            elif "anyOf" in value and "type" not in value:
                expanded_properties[name] = value
                expand_multi_type(expanded_properties[name],resolver)
            else:
                expanded_properties[name] = value
...
def match_step_to_regex(step):
    ...
    if isinstance(step, dict):
        ...
        if all(key in keys for key in ("enum", "type")) and step["type"] == "string":
            ...
            all(key in keys for key in ("type","pattern"))
            and step["type"] == "string"
        ):
